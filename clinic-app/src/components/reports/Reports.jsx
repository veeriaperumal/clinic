...
export default function Reports() {
  ...
  if (open) {
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-2xl mx-auto">
        <button onClick={() => setOpenId(null)} className="text-xs font-medium flex items-center gap-1 mb-4 text-primary hover:underline touch-manipulation">
          <ChevronLeft size={12} /> All reports
        </button>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }} className="p-4 sm:p-5 rounded-xl bg-surface border border-line shadow-card">
          <div className="flex items-start justify-between gap-3 mb-4">
            <div className="min-w-0">
              <h3 className="text-base font-semibold truncate">{open.test}</h3>
              <p className="text-xs text-ink-soft truncate">Collected {open.date} · Arogya Lab</p>
            </div>
          ...
          {open.params.length > 0 && (
            ...
            >
              <div className="grid grid-cols-4 gap-1 sm:gap-2 text-[10px] uppercase tracking-wide pb-1.5 mb-1 text-ink-soft font-semibold border-b border-line">
                <span>Parameter</span><span>Result</span><span>Range</span><span>Flag</span>
              </div>
              {open.params.map((p) => (
                <motion.div
                  key={p.name}
                ...
                >
                  ...
                  <span className="font-medium truncate">{p.result} {p.unit}</span>
          ...
          >
            {downloaded[open.id] ? <><Check size={14} /> Downloaded</> : <><Download size={14} /> Download PDF</>}
          </motion.button>
        </motion.div>
      </motion.div>
    );
  ...
  return (
    <div>
      <SectionHeading eyebrow="Lab & consult history" title="My reports" sub="Reports appear here as soon as the lab finishes processing." />
      <motion.div
        initial="hidden"
        animate="show"
        variants={{ hidden: { opacity: 0 }, show: { opacity: 1, transition: { staggerChildren: 0.04 } } }}
      ...
      >
        {REPORTS.map((r) => (
          ...
          >
            ...
            {r.status === "ready" && <ChevronLeft size={14} className="text-ink-soft rotate-180 shrink-0 hidden xs:block" />}
          </motion.button>
        ))}
      </motion.div>
    </div>
